...
build-production.js    esbuild bundler → self-contained dist/ (server + deps bundled)
public/
  index.html           shell
...
- **The `production` branch is a build artifact, not source.** `.github/workflows/production.yml` runs on every push to `main`: it runs `npm run build:production` (`build-production.js`), which bundles *both* the editor **and** `server.js` — inlining fastify/@fastify/static/chokidar via esbuild `platform: node, packages: bundle` — into a self-contained `dist/` (bundled `server.js` + `public/` + minimal `package.json` + README). The workflow publishes that `dist/` to the `production` branch. Pulling `production` gives a ready-to-run app: **no `npm install`, no `node_modules`** — just `node server.js` / `npm start`. `dist/` is gitignored (built, like `editor.bundle.js`). Two deliberate choices: the workflow commits `production` with **linear history** (a normal commit + push each build, never a force-push) so consumers can `git pull` for the latest; and `chokidar`'s optional native `fsevents` is marked esbuild-`external` (it can't be bundled — chokidar falls back to `fs.watch`, verified working incl. SSE). Never hand-edit `production`; changes there are overwritten on the next push to `main`. The GitHub repo must have Actions "Read and write permissions" enabled for the push to succeed.
...
- **Theming is token-driven** — `public/styles.css` defines all colors as CSS custom properties in `:root` (light) with a `:root[data-theme="dark"]` block overriding the same names for dark. Never hardcode a color in a rule that a theme needs to change; add/override a token instead. The header `#themeBtn` cycles system → light → dark (persisted as `nubsidian.theme`); `applyTheme()` in `app.js` resolves "system" against `matchMedia` and stamps a concrete `data-theme="light|dark"` on `<html>`, so CSS only sees the two concrete themes. An inline script in `index.html`'s `<head>` applies the same resolution before first paint to avoid a flash. A few surfaces are intentionally *not* themed: the sidebar and quick-switcher palette stay dark in both themes, and `.html-view` stays white (it renders arbitrary author-styled HTML that assumes a light page).
...
- **No database** — don't add one. Persistent state beyond file contents belongs in `config.json` if it's app-level config, or in the files themselves. The one exception is client-only UI state in `localStorage` (`nubsidian.collapsedRoots` for sidebar collapse, `nubsidian.recentFiles` for the recents list, `nubsidian.theme` for the light/dark/system theme mode) since those are UI preferences, not app config. Session restore isn't `localStorage`-based — it's the URL (see above).
